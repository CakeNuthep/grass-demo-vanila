...
export default class PointerLockControls extends EventDispatcher {
  constructor(camera, domElement, player) {
    ...
    this.onPointerlockErrorBind = this.onPointerlockError.bind(this);
  ...
  onPointerlockError(e) {
    console.error("THREE.PointerLockControls: Unable to use Pointer Lock API", e);
  }
  ...
  updateHeadBob(time, speed) {
    if (this.headBobActive) {
      ...
      const verticalBob = Math.sin(this.headBobTimer * verticalFrequency) * amplitude;
      const lateralBob = Math.cos(this.headBobTimer * lateralFrequency) * amplitude;
